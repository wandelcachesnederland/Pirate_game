...
import type { UpgradeId, UpgradeOffer } from '../game/types';
...
export function UpgradeScreen({ offers, wave, onChoose, isTouch }: Props) {
      ...
      <div className="flex min-h-full flex-col items-center justify-center gap-4 p-4 sm:gap-6">
        <div className="anim-pop text-center">
          <p className="font-fell text-base italic text-parch/90 sm:text-lg">
            Wave {wave} survived! The carpenter patched your hull.
          </p>
          <h2 className="title-gold text-5xl leading-none sm:text-7xl">Tortuga Shipwright</h2>
          <p className="mt-1 font-fell italic text-parch/80">Choose one improvement for your ship</p>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="font-pirate text-2xl leading-tight sm:text-[1.7rem]">{o.def.name}</div>
                  <div className="italic leading-snug text-ink-soft">{o.def.desc}</div>
                  <div className="mt-2 flex items-center gap-1 sm:justify-center">
                    ...
                    <span className="ml-1.5 text-xs italic opacity-70">
                      {o.level === 0 ? 'New' : `Lv ${o.level} → ${o.level + 1}`}
                    </span>
                  </div>
                </div>
        ...
        {!isTouch && (
          <p className="font-fell text-sm italic text-parch/70">
            Press <KeyCap>1</KeyCap> <KeyCap>2</KeyCap> <KeyCap>3</KeyCap> or click a card
          </p>
        )}
      </div>
    </div>
