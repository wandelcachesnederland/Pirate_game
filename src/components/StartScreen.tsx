...
import { ERA_FLAGSHIPS, ERA_SHIPS } from '../game/ships/era';
...
/** The four screens of putting to sea: title, sign-on, peril, era. */
type Step = 0 | 1 | 2 | 3;
  ...
  return (
    <div className="grid h-full place-items-center overflow-y-auto no-scrollbar p-2 sm:p-4">
      <div className="anim-pop w-full max-w-xl text-center">
        <div className="arcade-tag text-[0.6rem] sm:text-xs">Step 1 of 3 · Sign On</div>
...
/** Step three, filling the screen: the era, its waters and its hero hull. */
      ...
      <div className="flex shrink-0 flex-wrap items-end justify-between gap-x-2 gap-y-1">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <button
            ...
            className="flex cursor-pointer items-center gap-1.5 rounded-full border border-gold/50 bg-black/40 px-2.5 py-1 font-pirate text-base leading-none text-gold transition-colors hover:brightness-125"
          >
            {name.trim() || 'Nameless'}
 ...
 * big logo, then sign on, then peril under its own colours, then the era —
 * the chart you scroll through, the hero hull and her scouting report. Orders
 * and the hall of legends sit behind one button so the steps stay uncluttered.
 */
export function StartScreen({
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (step === 3 && !typing && (e.code === 'ArrowLeft' || e.code === 'ArrowRight')) {
        e.preventDefault();
        const idx = Math.max(
          0,
  ...
  const plate = (n: 1 | 2 | 3): 'todo' | 'active' | 'done' =>
      ...
      <div className="flex h-full flex-col">
        ...
        <header className="arcade-panel mx-2 mt-2 flex flex-wrap items-center justify-between gap-x-3 gap-y-1 px-2.5 py-1.5 sm:mx-3 sm:mt-3 sm:px-4">
          ...
          {step === 0 ? (
            ...
            <div className="order-last flex w-full items-center justify-center gap-1.5 sm:order-none sm:w-auto">
              <StepPlate n={1} label="Sign On" state={plate(1)} onClick={step > 1 ? () => go(1) : undefined} />
              <span className="font-pirate text-lg text-gold/70">›</span>
              <StepPlate n={2} label="Peril" state={plate(2)} onClick={step > 2 ? () => go(2) : undefined} />
              <span className="font-pirate text-lg text-gold/70">›</span>
              <StepPlate n={3} label="Era & Hero Ship" state={plate(3)} />
            </div>
          )}
          ...
          {step === 0 ? (
            ...
            <div className="flex flex-1 items-center justify-end gap-2 sm:flex-none">
              ...
              >
                ...
                {step === 1 ? 'Next: Peril ▸' : step === 2 ? 'Next: Era ▸' : 'Set Sail!'}
              </button>
            </div>
          )}
        </footer>
