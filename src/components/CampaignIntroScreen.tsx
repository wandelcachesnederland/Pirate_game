...
export function CampaignIntroScreen({ era, difficulty, name, onJump }: Props) {
  const { t } = useTranslation(['menu', 'common', 'peril', 'eras', 'regions']);
  ...
  return (
    <div className="flex h-full flex-col gap-2 overflow-y-auto no-scrollbar p-2 sm:gap-3 sm:p-3">
      <div className="flex flex-wrap items-end justify-between gap-x-3 gap-y-1">
        <div>
          <div className="arcade-tag text-[0.6rem] sm:text-xs">{t('menu:campaign.stepTag')}</div>
          <h2 className="arcade-marquee text-xl leading-none sm:text-3xl">{t('menu:campaign.heading')}</h2>
        </div>
      ...
      <div className="grid flex-1 gap-3 sm:grid-cols-[1.1fr_0.9fr] sm:gap-4">
        ...
        <div className="arcade-panel flex flex-col gap-3 p-3 sm:p-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="arcade-tag text-[0.6rem]">{t('menu:campaign.startsHere')}</div>
              <div className="font-pirate text-2xl leading-none text-parch sm:text-3xl">{t(`eras:${first.id}.name`)}</div>
              <div className="font-pirate text-sm text-gold">{first.year} · {t(`regions:arcade.${first.region}.name`)}</div>
            </div>
          </div>

          <div className="rounded-lg border border-parch/25 bg-black/30 p-2.5 text-sm leading-snug text-parch/90">
            <p className="italic">{t(`eras:${first.id}.blurb`)}</p>
            <p className="mt-2 text-[0.8rem] opacity-80">
              {t('menu:campaign.beginLine', { waves: CAMPAIGN_WAVES_PER_ERA })}
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="rounded-md border border-gold/30 bg-black/40 p-2">
              ...
              <div className="text-[0.65rem] uppercase tracking-wider opacity-70">{t('menu:campaign.erasStat')}</div>
            </div>
            <div className="rounded-md border border-gold/30 bg-black/40 p-2">
              <div className="font-pirate text-2xl leading-none text-gold">{CAMPAIGN_WAVES_PER_ERA}</div>
              <div className="text-[0.65rem] uppercase tracking-wider opacity-70">{t('menu:campaign.wavesStat')}</div>
            </div>
            <div className="rounded-md border border-gold/30 bg-black/40 p-2">
              <div className="font-pirate text-2xl leading-none text-gold">{totalWaves}</div>
              <div className="text-[0.65rem] uppercase tracking-wider opacity-70">{t('menu:campaign.totalStat')}</div>
            </div>
          </div>

          <div className="mt-auto flex items-center gap-2 text-[0.72rem] italic opacity-70">
            <Clock className="h-4 w-4" />
            <span>{t('menu:campaign.lockedLine', { era: t(`eras:${era}.name`), year: current.year })}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          <div className="arcade-panel p-2.5 sm:p-3">
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="font-pirate text-lg leading-none">{t('menu:campaign.route')}</span>
              <span className="arcade-tag ml-auto text-[0.6rem] opacity-70">{t('menu:campaign.stops', { n: allEras.length })}</span>
            </div>
              ...
              {allEras.map((e, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1 truncate font-pirate text-sm leading-tight">{t(`eras:${e.id}.name`)}</span>
          ...
          <div className="arcade-panel p-2.5 text-[0.78rem] italic leading-snug opacity-85">
            <p className="flex items-start gap-2">
              ...
              <span>{t('menu:campaign.keepLine', { n: allEras.length })}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <p className="shrink-0 text-center text-[0.64rem] italic leading-snug opacity-60">
        {t('menu:campaign.nextLine', { era: t(`eras:${first.id}.name`) })}
      </p>
    </div>
  );
}
